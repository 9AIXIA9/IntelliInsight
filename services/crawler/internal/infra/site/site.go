...
var (
	...
	siteMap   = map[proto.Site]domain.Site{
		proto.Site_XIAOHONGSHU: XHS("小红书"),
	}
	once sync.Once
)
...
func (X XHS) ParsePostDetail(html string, includeImages bool) (*proto.PostItem, error) {
	...
	// 从URL中提取帖子ID
	currentURL, _ := doc.Find("meta[property='og:url']").Attr("content")
	post.PostId = X.FormatPostID(currentURL)
	post.Url = currentURL

	// 标题
	post.Title = strings.TrimSpace(doc.Find(X.GetPostTitleSelector()).Text())

	// 内容
	post.Content = strings.TrimSpace(doc.Find(X.GetPostContentSelector()).Text())
